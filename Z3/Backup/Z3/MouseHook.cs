...
namespace Z3.Util
{
    public class MouseHook
    {
        ...
        private bool suspendKeyboard;

        public void Suspend()
        {
            suspendMouse = (hMouseHook != 0);
            suspendKeyboard = (hKeyboardHook != 0);
            Stop();
        }

        public void Resume()
        {
            Start(suspendMouse, suspendKeyboard);
        }
        ...
        private int MouseHookProc(int nCode, int wParam, IntPtr lParam)
        {
            ...
            if ((nCode >= 0) && (OnMouseActivity != null))
            {
                ...
                return CallNextHookEx(hMouseHook, -1, WM_MBUTTONUP, lParam); 
            }
            else
            {
                //call next hook
                //return CallNextHookEx(hMouseHook, nCode, wParam, lParam);
                //if(clickCount > 0 && Button == MouseButtons.Right)
                //return 1;
                //else
                return CallNextHookEx(hMouseHook, nCode, WM_MBUTTONUP, lParam);
            }
        }
        ...
        private int KeyboardHookProc(int nCode, Int32 wParam, IntPtr lParam)
        {
            ...
            if ((nCode >= 0) && (KeyDown != null || KeyUp != null || KeyPress != null))
            {
                ...
                if (KeyPress != null && wParam == WM_KEYDOWN)
                {
                    ...
                    bool isDownCapsLock2 = (0 != (GetKeyState(VK_CAPITAL) & 1));
                ...
                if (KeyUp != null && (wParam == WM_KEYUP || wParam == WM_SYSKEYUP))
                {
                    ...
                    bool isDownAlt = ((GetKeyState(VK_MENU) & 0x80) == 0x80);

                    if (isDownCapslock && keyData >= Keys.A && keyData <= Keys.Z)
                        keyData |= Keys.Shift;
                    if (isDownShift)
                        keyData ^= Keys.Shift;
                    if (isDownAlt)
